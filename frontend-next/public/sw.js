    ...
    try {
        ...
        try {
            ...
            return {
                ...
                data: j.data && typeof j.data === "object"
                    ? j.data
                    : typeof j.url === "string"
                        ? { url: j.url }
                        : {},
...
self.addEventListener("push", (event) => {
    event.waitUntil(parsePushPayload(event).then(({ title, body, tag, data, playSound }) => {
...
self.addEventListener("message", (event) => {
    ...
    if (data?.type === "vega-prime-sound") {
        event.waitUntil(fetch(SOUND_URL).catch(() => undefined).then(() => notifyClientsPlaySound()));
    }
});
export {};
