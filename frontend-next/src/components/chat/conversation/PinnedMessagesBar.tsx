...
export default function PinnedMessagesBar({
  ...
  return (
    <div
      dir={rtl ? "rtl" : "ltr"}
      className="flex w-full items-center gap-1 border-b border-brand-200/45 bg-brand-50/50 px-2 py-2 dark:border-white/10 dark:bg-white/[0.04]"
    >
      {multi ? (
        <button
          type="button"
          onClick={() => setIndex((i) => (i <= 0 ? pinnedMessages.length - 1 : i - 1))}
          className="shrink-0 rounded-full p-1.5 text-brand-700 hover:bg-brand-100/80 dark:text-white/80 dark:hover:bg-white/10"
      ...
      >
        <Pin className="h-3.5 w-3.5 shrink-0 text-brand-600 dark:text-white/75" aria-hidden />
        <span className="font-semibold text-brand-800 dark:text-white/90">
          {t("pinnedInChat")}
          {multi ? ` (${safeIndex + 1}/${pinnedMessages.length})` : ""}
        </span>
        <span className="min-w-0 flex-1 truncate text-ink/90 dark:text-white/75">
          {preview}
          {fullLen > 120 ? "…" : ""}
        </span>
      </button>
